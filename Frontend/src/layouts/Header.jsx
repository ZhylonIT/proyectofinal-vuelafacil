...
function Header() {
  ...
  const navItems = isAuthenticated
    ? [
        ...
        ...(user?.role === 'admin' ? [{ label: 'Administración', path: '/administracion' }] : [])
      ]
    : [
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 2, alignItems: 'center' }}>
            ...
            {!isAuthenticated ? (
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, ml: 2, pl: 2, borderLeft: '1px solid rgba(0,0,0,0.12)' }}>
                ...
                <Typography sx={{ color: '#0B3B5A', fontWeight: 600, fontSize: '1rem', maxWidth: 150, noWrap: true }}>{user?.firstName}</Typography>
